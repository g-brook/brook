    ...
    <div class="max-w-6xl mx-auto p-1 space-y-4 fade-in">
      ...
      <div class="bg-base-200/40 rounded-3xl p-6 border border-base-content/5 space-y-6 shadow-sm mx-1">
        <div class="flex items-center justify-between">
          ...
          <div v-if="tokenInfo.token" class="flex gap-2">
            <button @click="copyToken"
                    class="btn btn-ghost btn-xs h-8 px-3 font-black uppercase tracking-widest hover:bg-primary hover:text-primary-content transition-all">
        ...
        <div v-if="tokenInfo.token" class="space-y-4">
          <div class="form-control w-full">
            <div class="relative group">
              <input :value="showToken ? tokenInfo.token : maskedToken" type="text"
                     class="input input-bordered focus:input-primary w-full h-11 font-mono text-sm font-black tracking-tight bg-base-100/30 hover:bg-base-100/50 focus:bg-base-100 transition-all shadow-sm border-base-content/5 pr-24"
                     readonly/>
              ...
              <div class="absolute right-1 top-1 join">
                <button class="btn btn-ghost btn-sm h-9 join-item px-3 hover:bg-base-content/5"
                  ...
                  <Icon :icon="showToken ? 'brook-eye-close' : 'brook-eye'" style="font-size: 16px;"
                        class="opacity-40"/>
                </button>
              </div>
            </div>
          ...
          <div class="flex gap-3 pt-2">
            <button @click="generateToken"
                    class="btn btn-primary h-11 flex-1 font-black uppercase tracking-widest shadow-lg shadow-primary/20"
            ...
            <button @click="revokeToken"
                    class="btn btn-error btn-outline h-11 px-6 font-black uppercase tracking-widest border-2 hover:border-error"
                    :disabled="isRevoking">
        ...
        <div v-else class="text-center py-12 bg-base-100/30 rounded-2xl border border-dashed border-base-content/10">
          ...
          <p class="text-xs font-medium opacity-40 leading-relaxed mb-8 max-w-xs mx-auto">{{
              t('mysetting.noTokenDesc')
            }}</p>

          <button @click="generateToken"
                  class="btn btn-primary btn-md gap-3 px-10 shadow-xl shadow-primary/20 font-black uppercase tracking-widest text-xs"
